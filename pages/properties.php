...
if (isset($_GET['view'])) {
    ...
    $activeTenancy = Database::fetchOne('SELECT * FROM tenancies WHERE property_id=? AND tenant_id=? AND status="active" AND deleted_at IS NULL ORDER BY id DESC LIMIT 1', [$property['id'], $_tenantId]);
    $period       = date('Y-m');
